...
export default function DashboardPage() {
  ...
  // Layer paint configs (styled as requested: cyan for optimal, orange for blocked)
  ...
  return (
    <div className="dash-v2" style={{ backgroundColor: isDark ? '#060d1f' : '#f8fafc', color: textUI }}>
      ...
      <div className="dash-filterbar" style={{ backgroundColor: isDark ? 'rgba(15,23,42,0.98)' : '#ffffff', borderBottom: `1px solid ${borderUI}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', height: '52px', backdropFilter: 'blur(12px)', zIndex: 50, position: 'relative' }}>
        ...
        <div ref={searchContainerRef} style={{ position: 'relative', zIndex: 300 }}>
          <form onSubmit={handleSearch} style={{ display: 'flex', alignItems: 'center', backgroundColor: isDark ? '#0f172a' : '#f1f5f9', borderRadius: showSuggestions ? '8px 8px 0 0' : '8px', border: `1px solid ${showSuggestions ? '#3b82f6' : borderUI}`, padding: '6px 12px', width: '340px', transition: 'all 0.2s' }}>
            ...
            />
            {searchQuery && (
              <button type="button" onClick={() => { setSearchQuery(''); setSearchSuggestions([]); setShowSuggestions(false) }} style={{ background: 'transparent', border: 'none', color: textSubUI, cursor: 'pointer', fontSize: '18px', lineHeight: 1, padding: '0 2px' }}>├ù</button>
            )}
          </form>
